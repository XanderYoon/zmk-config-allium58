...
static void draw_top(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
    ...
    init_label_dsc(&label_dsc_profile, LVGL_FOREGROUND, &lv_font_montserrat_18,
    ...
    // Draw selected Bluetooth profile
    lv_canvas_draw_rect(canvas, 0, 21, 68, 42, &rect_white_dsc);
    lv_canvas_draw_rect(canvas, 1, 22, 66, 40, &rect_black_dsc);

    char profile_text[6] = {};
    snprintf(profile_text, sizeof(profile_text), "BT %d", state->active_profile_index + 1);
    lv_canvas_draw_text(canvas, 0, 33, 68, &label_dsc_profile, profile_text);
